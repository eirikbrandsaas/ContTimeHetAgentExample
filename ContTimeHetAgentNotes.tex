...
In this lecture note we will write down a continuous time variant of the Aiyagari-Bewley-Hugget type models, solve it using finite difference upwinding and extend the model to include housing. This notes will rely heavily on Achdou et al (2017). 

\subsection{Preview of Results}
At the end of this lecture you will be able to `instantly' solve equilibrium models with heterogeneous agents, and find the joint income-wealth distributions. In addition we will deal with non-convexities, models with multiple stationary distributions and even a model with heterogeneous families consisting of multiple altruistic households interacting without commitment - all solved using the same numerical method.
...
\begin{equation*}
	\partial_{C}V_{i,j}1=\frac{V_{i+1,j}-V_{i-1,j}}{\Delta a}.
\end{equation*}
...
The dashed vertical line represents the first wealth level you can meet the downpayment requirement. }
\end{figure}
...
\begin{exercise}
Rewrite the program that is posted on the webpage to include a switch so that you can use either the implicit or explicit method. Time the different methods, which one is faster? Which can you `break' easier (e.g. making the grid too sparse, increasing $\gamma$)?
\end{exercise}
\end{document}